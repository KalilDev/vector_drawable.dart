...
void main() {
  for (final file in Directory('/home/pedro/Imagens/uno/vd').listSync()) {
    final c = (file as File).readAsStringSync();
    final d = XmlDocument.parse(c);
    final vd = VectorDrawable.parseDocument(d,
        ResourceReference('uno', file.path.split('/').last.split('.').first));
    print('final ${vd.source!.name} = ' +
        CodegenVectorDrawableVisitor().visitVectorDrawable(vd).toString() +
        ';\n');
  }
  //exit(0);
  ...
  Widget build(BuildContext context) => FilledCard(
        ...
        child: Column(
          children: [
            ...
            _HorizontalCenter(
                child: widget.file.unwrap.resolvedAnimatedVectorSnap
                    .map((snap) => snap.hasData
                        ...
                        : snap.hasError
                            ? widget.file.unwrap.resolvedVectorSnap
                                .map((snap) => snap.hasData
                                    ? VectorWidget(
                                        vector: snap.requireData.body,
                                      )
                                    : snap.hasError
                    ...
                    .build()),
            _ExpansionAnimation(
              isExpanded: widget.file.unwrap.resolvedAnimatedVectorSnap
                  .map((snap) => snap.hasData),
...
class FileController
  ...
  final ValueNotifier<Directory?> _root;
  final ValueNotifier<bool?> _isAnimatedVector;
  final ActionNotifier _didDelete = ActionNotifier();
  ...
  FileController(File file, Directory? root, AsyncResourceResolver? resolver)
      : file = file,
        ...
        _root = ValueNotifier(root),
        _isAnimatedVector = ValueNotifier(null);

  late final ValueListenable<AsyncSnapshot<AnimatedVectorDrawable>>
      _resolvedAnimatedVector;
  late final ValueListenable<AsyncSnapshot<AnimatedVectorDrawable>>
      _parsedAnimatedVectorSnap;

  late final ValueListenable<AsyncSnapshot<VectorDrawable>> _resolvedVector;
  late final ValueListenable<AsyncSnapshot<VectorDrawable>> _parsedVectorSnap;

  ValueListenable<AsyncSnapshot<AnimatedVectorDrawable>>
      get parsedAnimatedVectorSnap => _parsedAnimatedVectorSnap.view();
  ValueListenable<AsyncSnapshot<AnimatedVectorDrawable>>
      get resolvedAnimatedVectorSnap => _resolvedAnimatedVector.view();
  ValueListenable<AsyncSnapshot<VectorDrawable>> get parsedVectorSnap =>
      _parsedVectorSnap.view();
  ValueListenable<AsyncSnapshot<VectorDrawable>> get resolvedVectorSnap =>
      _resolvedVector.view();
  ValueListenable<AsyncSnapshot<String>> get animatedVectorGeneratedCode =>
      resolvedAnimatedVectorSnap.map((e) => e
  ...
  late final ValueListenable<AsyncSnapshot<String>> _formattedGeneratedAVDCode =
      animatedVectorGeneratedCode.map((code) => code.map(_formatStatement));
  ValueListenable<AsyncSnapshot<String>> get formattedGeneratedAVDCode =>
      _formattedGeneratedAVDCode.view();
  ValueListenable<AsyncSnapshot<String>> get vectorGeneratedCode =>
      resolvedVectorSnap.map((e) => e
          .map((drawable) => CodegenVectorDrawableVisitor()
              .visitVectorDrawable(e.requireData)
              .toString())
          .map((code) => 'final ${ref.name} = $code;'));
  // late final because _formatCode is expensive
  late final ValueListenable<AsyncSnapshot<String>> _formattedGeneratedVDCode =
      animatedVectorGeneratedCode.map((code) => code.map(_formatStatement));
  ValueListenable<AsyncSnapshot<String>> get formattedGeneratedVDCode =>
      _formattedGeneratedVDCode.view();
  ValueListenable<AsyncSnapshot<String>> get formattedGeneratedCode =>
      _isAnimatedVector.view().bind((isAnimatedVector) =>
          isAnimatedVector == true
              ? formattedGeneratedAVDCode
              : formattedGeneratedVDCode);
  ...
  void init() {
    ...
    _parsedAnimatedVectorSnap = file
    ...
    _parsedVectorSnap = file
        ...
        .bind((updatedFile) => updatedFile
            ...
            .then(XmlDocument.parse)
            .then((doc) => VectorDrawable.parseDocument(doc, ref),
                onError: (_) => _isAnimatedVector.value = true)
            .then((_) {
              print('avd parsed');
              return _;
            })
        ...
        .tap((e) => 'bound avd converted')
        .view();
    _resolvedAnimatedVector = _resolver
        ...
        .bind<AsyncSnapshot<AnimatedVectorDrawable>>((resolver) =>
            resolver == null
                ...
                : parsedAnimatedVectorSnap.bind((snap) => snap.hasData
                    ? _resolveAnimatedVector(snap.requireData, resolver)
                    : SingleValueListenable(snap)))
        .tap((e) => print('avd snapshot: $e'));
    _resolvedVector = parsedVectorSnap;
  }

  ResourceReference get ref => ResourceReference(
      p.basename(p.dirname(file.path)), p.basenameWithoutExtension(file.path));

  ValueListenable<AsyncSnapshot<AnimatedVectorDrawable>> _resolveAnimatedVector(
...
class ResourceController extends ControllerBase<ResourceController> {
  ...
  final SortedAnimatedListController<FileController> _files;
  ...
  ResourceController(
    ...
    Map<String, Directory> namespaces,
        ...
        _files = SortedAnimatedListController.from(
  ...
  late final ValueListenable<AsyncSnapshot<String>> _generatedCode =
    ...
    (e) => e
        .fold<ValueListenable<AsyncSnapshot<List<String>>>>(
          ...
          (acc, e) => acc.bind((acc) => e.animatedVectorGeneratedCode.map(
  ...
  ControllerHandle<SortedAnimatedListController<FileController>>
      get avdFilesController => _files.handle;
  ...
  void _registerFileController(FileController file) {
    file.didDelete.listen(() => _files.remove(file));
  ...
  void addFile(File file) {
    ...
    _files.insert(addSubcontroller(controller));
  ...
  void init() {
    super.init();
    for (final controller in _files.values.value) {
    ...
    _files.init();
    _files.didDiscardItem.tap(_deleteFileController);
    _namespaces.init();
    _namespaces.didDiscardItem.tap(_deleteNamespace);
  }

  void dispose() {
    IDisposable.disposeAll(_files.values.value);
